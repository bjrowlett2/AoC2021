...
fn main() {
    ...
    for line in aoc::lines("inputs/day_02.txt") {
        match parse_command(&line) {
            Ok(value) => day.commands.push(value),
            Err(reason) => panic!("parse_command failed: {}", reason),
        };
    }
...
fn parse_amount(amount: &str) -> i32 {
    return match amount.parse::<i32>() {
        Ok(value) => value,
...
fn parse_command(line: &str) -> Result<Command, String> {
    ...
    return Ok(Command {
        action: match parts.next() {
            ...
            None => return Err(String::from("No action found")),
        },
        amount: match parts.next() {
            Some(value) => parse_amount(&value),
            None => return Err(String::from("No amount found")),
        },
    });
}

fn solve_part_1(day: &Day02) -> Result<i32, String> {
    let mut depth = 0;
...
fn solve_part_2(day: &Day02) -> Result<i32, String> {
    let mut aim = 0;
    let mut depth = 0;
    let mut horizontal = 0;
